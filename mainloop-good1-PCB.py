...
class Voice:
    ...
    def __init__(self, mods, retune=True, cutoff_freq_tracking=True):
        ...
        self.xfade_idx = 0  # TODO - this is actually EXTERNAL MIX
        ...
        # these names MUST MATCH the actual class property names
    ...
    def send(self, note_down, midinote=None):
        ...
        if note_down and not self.note_down:
            ...
            if midinote:
                ...
                print("volts per octave:", self.coarse, self.fine)
...
#TEST_AEN_PIN = Pin(27,Pin.OUT,value=0)  # always keep low and this is managed by DAC module now
# TUNE_LATCH_PIN = Pin(26,Pin.OUT,value=1)  # TEMP - managed by mydacs libara y TEMP

"""  # not used because we are now using the actual CS pin
...
# NEW SORT ORDER: ["xfade", "suboctave", "envelope", "pwm", "coarse", "fine", "cutoff", "resonance"]

"""
tempmodlist = [
                [LFOLIST[6],ADSRLIST[6]], # c   XFADE
               [LFOLIST[7],ADSRLIST[7]], # f    SUB
               [LFOLIST[5],ADSRLIST[5]], # xfade ENV
               [LFOLIST[1],ADSRLIST[1]],  # suboct PWM
               [LFOLIST[4],ADSRLIST[4]],  # pwm COARSE
               [LFOLIST[0],ADSRLIST[0]],  # VCA FINE
               [LFOLIST[3],ADSRLIST[3]],  # resn CO
               [LFOLIST[2],ADSRLIST[2]]  # cutoff RES
               ]
"""
tempmodlist = [
    [],
...
V = Voice(tempmodlist, retune=True, cutoff_freq_tracking=True)
VOICES = [V]  # in the future, there be more

# these are the names as they appear on the LCD
# 6 DEFO doing envelope.
...
try:
    ...
    send_dac_value(2, 0)  # manually turn off single voice's VCA
    for q in VOICES:
        q.osc.save_arrays()
